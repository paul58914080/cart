...
public class CartJpaTest {
  ...
  public void shouldGiveMeCartsWhenAskedGivenCartExistsInDatabase() {
    // Given from @Sql
    // When
    List<Cart> carts = obtainCart.getAllCarts();
    // Then
  ...
  public void shouldGiveTheCartWhenAskedByIdGivenThatCartByThatIdExistsInDatabase() {
    // Given from @Sql
    // When
    Optional<Cart> cart = obtainCart.getCartByCode(1L);
    // Then
    assertThat(cart).isNotNull().isNotEmpty().get().isEqualTo(Cart.builder().code(1L).description("Twinkle twinkle little star").build());
  }

  @Sql(scripts = {"/sql/data.sql"})
  @Test
  @DisplayName("given carts exists in database when asked for cart by id that does not exist should give empty")
  public void shouldGiveNoCartWhenAskedByIdGivenThatCartByThatIdDoesNotExistInDatabase() {
    // Given from @Sql
    // When
    Optional<Cart> cart = obtainCart.getCartByCode(-1000L);
    // Then
